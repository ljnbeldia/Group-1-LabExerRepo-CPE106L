...
class ATM(EasyFrame):
    ...
    def __init__(self, bank):
        ...
        self.bank = bank
        self.account = None
        # Added to track failed attempts to login.
    ...
    def login(self):
        ...
        if self.account:
            ...
            # added to reset failed attempts on successful login
            self.failedAttempts = 0
        else:
            #removed self.statusField.setText("Name and pin not found!") and added the following:
            self.failedAttempts += 1
            self.statusField.setText("Login failed. Please try again.") 
            if self.failedAttempts >= 3:
                self.messageBox(title="Warning", message="Police will be called! Too many failed login attempts.")
